...
const handleResetPassword = async () => {
  ...
  try {
    ...
    if (response.data.status === 200) {
      ...
      passwordError.value = response.data.message.password || 'An error occurred.';
    }
  } catch (error) {
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <div class="border mt-16 sm:mx-auto sm:w-full sm:max-w-md p-4 rounded-lg">
        <div class="">
          ...
          <h2 class="mt-5 text-center text-3xl font-extrabold text-gray-900">Reset Your Password</h2>
        </div>

        <div class="mt-10 ">
          <form @submit.prevent="handleResetPassword" class="space-y-6">
            <div>
              <label class="block text-sm font-medium text-gray-700">Enter New Password <span class="text-red-500">*</span></label>
              <input id="password" v-model="password" type="password" placeholder="*********"  class="mt-1 block w-full p-3 border border-gray-300 rounded-md" required>
            </div>
